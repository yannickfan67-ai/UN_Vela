...
static void history_push(const char *url){
    ...
    if(g_history_count<VELA_HISTORY_MAX){g_history_index=g_history_count++;copy(g_history[g_history_index],VELA_URL_CAP,url);}
    else {for(int i=1;i<VELA_HISTORY_MAX;i++)copy(g_history[i-1],VELA_URL_CAP,g_history[i]);g_history_index=VELA_HISTORY_MAX-1;copy(g_history[g_history_index],VELA_URL_CAP,url);}
}
static int navigate(const char *url,int push){
    ...
    copy(g_status,sizeof(g_status),"Loading...");request_repaint();
    if(!g_platform.http_get(g_platform_context,url,g_raw,sizeof(g_raw),g_status,sizeof(g_status)))return 0;
    set_url(url);parse_current();if(push)history_push(url);
...
int vela_set_platform(const VelaPlatformOps *platform,void *context){
    bytes_zero(&g_platform,sizeof(g_platform));g_platform_context=context;g_platform_ready=0;
    if(!platform)return 1;if(!vela_platform_is_compatible(platform))return 0;
    size_t n=platform->struct_size<sizeof(g_platform)?platform->struct_size:sizeof(g_platform);bytes_copy(&g_platform,platform,n);g_platform_ready=1;return 1;
}
...
int vela_init_ex(int viewport_width,const VelaPlatformOps*platform,void*context){
    g_viewport=viewport_width>120?viewport_width:120;g_scroll=0;g_history_count=0;g_history_index=-1;set_url("");
    if(!vela_set_platform(platform,context))return 0;copy(g_status,sizeof(g_status),"Ready / Aster Engine ");append(g_status,sizeof(g_status),aster_version());start_page();return 1;
}
...
void vela_set_scroll(int y){int max=g_doc.document_height>0?g_doc.document_height-1:0;if(y<0)y=0;if(y>max)y=max;g_scroll=y;request_repaint();}
